...
def get_latent_env(env_class):
    ...
    class TMP(env_class):
        @classmethod
        def default_config(cls):
            config = super(TMP, cls).default_config()
            config.update(dict(enable_latent=False, latent_dim=-1))
            return config

        def __init__(self, config=None):

            super(env_class, self).__init__(config)
            # self.latent_map = {}
        ...
        def _add_latent(self, obs, agent_name):
            if not self.config["enable_latent"]:
                return obs
            # if agent_name not in self.latent_map:
            ...
            #     latent = (latent + 1) / 2
            #
            #     self.latent_map[agent_name] = latent
...
def get_gym_compatible_env(env_class):
    return createGymWrapper(env_class)
